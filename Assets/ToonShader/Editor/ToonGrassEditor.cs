...
public class ToonGrassEditor : ShaderGUI
{
	...
	public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
	{
		...
		else
		{
			...
			Texture titleTex = AssetDatabase.LoadAssetAtPath(GetAssetPath() + "ToonGrass.png", typeof(Texture2D)) as Texture2D;//Get Title image
			...
			speed = EditorGUILayout.FloatField("Waving Speed", targetMat.GetFloat("_Speed"));
			wavingStrength = EditorGUILayout.Slider("Waving Strenght", targetMat.GetFloat("_Waving"), 0, 1);
			if(targetMat.GetFloat("_BlendVetrexColors") == 0) { blendVertexColors = false; } else if(targetMat.GetFloat("_BlendVetrexColors") == 1) { blendVertexColors = true; }
			blendVertexColors = EditorGUILayout.Toggle("Blend using Vertex Colors",blendVertexColors);
			GUILayout.Space(10);
			#endregion
			#region Textures
			...
			if (EditorGUI.EndChangeCheck() || (Event.current.type == EventType.ValidateCommand && Event.current.commandName == "UndoRedoPerformed"))
			{
				...
				targetMat.SetFloat("_Waving", wavingStrength);
				if(blendVertexColors == true) { targetMat.SetFloat("_BlendVetrexColors", 1); } else { targetMat.SetFloat("_BlendVetrexColors", 0); }
