...
public class VCCalculator : MonoBehaviour
{

	public enum Method { Simple, AngularDeviation };

	public Method method = Method.Simple;
	...
	void OnDrawGizmos()
	{
		if (calculateNow && !skinned)
		{
			calculateNow = false;
			TryGenerate();
		}
		else if (calculateNow && skinned)
		{
			calculateNow = false;
			TryGenerateSkinned();
		}
	...
	void Awake()
	{
		if (calculateOnAwake && !skinned)
		{
			TryGenerate();
		}
		else if (calculateOnAwake && skinned)
		{
			TryGenerateSkinned();
		}
		/*
	...
	void TryGenerateSkinned()
	{
		SkinnedMeshRenderer mf = GetComponent<SkinnedMeshRenderer>();
		if (mf == null)
		{
			Debug.LogError("MeshFilter missing on the vertex color generator Skinned", gameObject);
			return;
		}
		if (mf.sharedMesh == null)
		{
			Debug.LogError("Assign a mesh to the MeshFilter before generating vertex colors Skinned", gameObject);
			return;
		}
		Generate(mf.sharedMesh);
		Debug.Log("Vertex colors generated Skinned", gameObject);
	}
	void TryGenerate()
	{
